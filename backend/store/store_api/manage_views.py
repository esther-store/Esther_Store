...
class ProductsManagment(viewsets.ModelViewSet):
    ...
    filterset_fields = ['id', 'categoria', 'precio',
                        "recommended", "is_active", "promotion"]
    ...
    def delete(self, request, pk=None):
        try:
            ...
            deleted_products = Producto.objects.filter(
                id__in=products_to_delete).delete()
    ...
    def quit_products_category(self, request):
        ...
        if not all(isinstance(product_id, int) for product_id in products):
            return Response({"message": "Invalid product IDs."}, status=status.HTTP_400_BAD_REQUEST)
        try:
            updated_count = Producto.objects.filter(
                id__in=products).update(categoria=None)
            if updated_count == 0:
    ...
    def quit_products_promotion(self, request):
        ...
        try:
            updated_count = Producto.objects.filter(
                id__in=products).update(promotion=None)
            if updated_count == 0:
                return Response({"message": "No products were removed. Check if the product IDs are correct."}, status=status.HTTP_400_BAD_REQUEST)
            return Response([], status=status.HTTP_200_OK)
        except:
...
class ManageCategories(viewsets.ModelViewSet):
    ...
    def delete(self, request):
        try:
            ...
            if categories_to_delete == [] or categories_to_delete == None:
                return super().delete(request)
            try:
                updated_count = Categoria.objects.filter(
                    id__in=categories_to_delete).delete()
                if updated_count == 0:
    ...
    def add_products_to_category(self, request, pk):
        ...
        try:
            updated_count = Producto.objects.filter(
                id__in=products).update(categoria=pk)
            if updated_count == 0:
    ...
    def remove_products_from_category(self, request, pk):
        ...
        try:
            updated_count = Producto.objects.filter(
                id__in=products, categoria=pk).update(categoria=None)
            if updated_count == 0:
                return Response({"message": "No products were removed. Check if the product IDs are correct."}, status=status.HTTP_400_BAD_REQUEST)
            return Response([], status=status.HTTP_200_OK)
        except:
...
class PromotionsManagment(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        # Convert is_special to booleano
        ...
        if Promotion.objects.filter(is_special=True).count() >= 4 and is_special:
    ...
    def update(self, request, pk=None):
        ...
        # Check if it's changing from no special to special
        if is_special and not instance.is_special:
            special_count = Promotion.objects.filter(is_special=True).count()
            if special_count >= 4:
                return Response(
                    {"data": [], "message": "Maximum number of special promotions reached"},
    ...
    def delete(self, request):
        try:
            ...
            if promotions_to_delete == [] or promotions_to_delete == None:
                return super().delete(request)
            try:
                updated_count = Promotion.objects.filter(
                    id__in=promotions_to_delete).delete()
                if updated_count == 0:
    ...
    def add_products_to_promotion(self, request, pk):
        ...
        try:
            updated_count = Producto.objects.filter(
                id__in=products).update(promotion=pk)
            if updated_count == 0:
                return Response({"message": "No products were added. Check if the product IDs are correct."}, status=status.HTTP_400_BAD_REQUEST)
            return Response([], status=status.HTTP_200_OK)
        except:
    ...
    def remove_products_from_promotion(self, request, pk):
        ...
        try:
            updated_count = Producto.objects.filter(
                id__in=products, promotion=pk).update(promotion=None)
            if updated_count == 0:
                return Response({"message": "No products were removed. Check if the product IDs are correct."}, status=status.HTTP_400_BAD_REQUEST)
            return Response([], status=status.HTTP_200_OK)
        except:
