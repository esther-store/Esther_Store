import "../pagesStyles/ManagementUsers.css";
...
import { useGetUsers } from "@/hooks/managementHooks/useGetUsers";
...
function ManagementSecurity() {
  ...
  const { loading, setLoading } = useGetUsers({
    searchParams: searchParams,
    setUsers: setDataUsers,
  });
  ...
  const confirm2 = (id) => {
    confirmDialog({
      ...
      accept: () => {
        setLoading(true);
        deleteUser({ users: [id], token: auth.token })
          .then(() => {
